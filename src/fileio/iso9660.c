...
#define DVDBUFFER 2048
static char dvdbuffer[DVDBUFFER] ATTRIBUTE_ALIGN (32);
PVD pvd;
...
****************************************************************************/
static void FlushXenoCache( void )
{
  u8 *cache;
...
mount_image (void)
{
  ...
  while (sector < 64)
    {
      while( !gcsim64 (dvdbuffer, (long long int)sector * 2048, 2048) );
  ...
  pvd.rootoffset = pvd.rootoffset * 2048;
  pvd.pathtableoffset = pvd.pathtableoffset * 2048;

  pvd.pathtablelength &= ~0x1f;
  pvd.pathtablelength += 64;

  pathtable = memalign (32, pvd.pathtablelength);
  memset (pathtable, 0, pvd.pathtablelength);

  while ( !gcsim64 (pathtable, (long long int)pvd.pathtableoffset, pvd.pathtablelength) );
...
DecodePathTable (void)
{
  ...
  pathdecs = memalign (32, pathentries * sizeof (PATHDECODE));
  ...
  if (d >= 0)
    {
      ...
      if (prevoffset64 != pdec[d - 1].offset64)
        {
          memset (dvdbuffer, 0, DVDBUFFER);
          while( !gcsim64 (dvdbuffer, pdec[d - 1].offset64, DVDBUFFER) );
      ...
      while (fdir->nlength)
        {
          ...
          if (fdir->nlength == 0)
            {
              /*** Read next sector ***/
              dvdoffset64 += DVDBUFFER;
              while( !gcsim64 (dvdbuffer, pdec[d-1].offset64 + dvdoffset64, DVDBUFFER ) );
              ofs = 0;
