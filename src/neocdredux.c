...
char *neogeo_ipl_memory = NULL;
char *neogeo_spr_memory = NULL;
char *neogeo_pcm_memory = NULL;
char *neogeo_all_memory = NULL;
...
static unsigned char config_game_name[80];
...
extern u32 SYS_GetArenaHi(void);
extern u32 SYS_GetArenaLo(void);
...
int neogeo_redux(void)
{
	...
	/*** Using GC memory card ***/
    memset(neogeo_memorycard, 0, 8192);
    while (neogeo_get_memorycard() == 0)
	ActionScreen((char *) "Please insert memory card");
...
#else
#if USESD
    // Need to put the SD mounting stuff here :D
    SD_Mount();
#else
    bannerscreen();
#endif
    ...
    if (!fp) {
	InfoScreen((char *) "NO BIOS AVAILABLE!");
	while (1);
    }

    GEN_fread(neogeo_rom_memory, 1, ROM_MEM, fp);
    GEN_fclose(fp);
