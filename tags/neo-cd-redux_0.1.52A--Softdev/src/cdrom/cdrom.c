...
static void cdrom_apply_patch(unsigned short *source, int offset, int bank);
  ...
  do
    {
      ...
      ***/
      Readed = GEN_fread(Ptr, 1, BUFFER_SIZE, fp);
      Ptr += Readed;
      totalbytes += Readed;
    }
...
static int cdrom_load_z80_file(char *FileName, unsigned int Offset)
{
  ...
  totalbytes = GEN_fread(subcpu_memspace + Offset, 1, 0x10000, fp);
  GEN_fclose(fp);
...
void cdrom_apply_patch(unsigned short *source, int offset, int bank)
{
  int master_offset;
