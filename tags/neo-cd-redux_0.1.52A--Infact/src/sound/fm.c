...
static void
init_timetables(FM_ST * ST, u8 * DTTABLE, int ARRATE, int DRRATE)
{
    int i, d;
    double rate;
...
static void FMsave_state_channel(FM_CH * CH, int num_ch)
{
	...
	create_state_register(ST_YM2610_FM, "op1_out1", ch + 1,
			      &CH->op1_out[0], 1 * sizeof(s32), REG_INT32);
	create_state_register(ST_YM2610_FM, "op1_out2", ch + 1,
			      &CH->op1_out[1], 1 * sizeof(s32), REG_INT32);
...
static void FMsave_state_st(FM_ST * ST)
{
    create_state_register(ST_YM2610_FM, "address", 1, &ST->address,
			  1 * sizeof(u8), REG_u8);
    create_state_register(ST_YM2610_FM, "IRQ", 1, &ST->irq, 1 * sizeof(u8),
			  REG_u8);
...
static void OPNSetPris(FM_OPN * OPN, int pris, int TimerPris, int SSGpris)
{
	...
	OPN->FN_TABLE[i] =
	    (u32) ((double) i * OPN->ST.freqbase * FREQ_RATE * (1 << 7) /
		      2);
    }
#if FM_LFO_SUPPORT
    /* LFO freq. table */
...
//    create_state_register(ST_YM2610, "ym2610"   ,1, F2610   , sizeof(YM2610));
    create_state_register(ST_YM2610, "out_ch", 1, out_ch, 4 * sizeof(s32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in1", 1, &pg_in1, 1 * sizeof(s32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in2", 1, &pg_in2, 1 * sizeof(s32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in3", 1, &pg_in3, 1 * sizeof(s32),
			  REG_INT32);
    create_state_register(ST_YM2610, "pg_in4", 1, &pg_in4, 1 * sizeof(s32),
			  REG_INT32);


#if 1
    create_state_register(ST_YM2610, "regs", 1, F2610->REGS,
			  512 * sizeof(u8), REG_u8);
    FMsave_state_st(&FM2610.OPN.ST);
    FMsave_state_channel(FM2610.CH, 6);
    /* 3slots */
    create_state_register(ST_YM2610, "slot3fc", 1, F2610->OPN.SL3.fc,
			  3 * sizeof(u32), REG_u32);
    ...
    create_state_register(ST_YM2610, "arrivedFlag", 1,
			  &F2610->adpcm_arrivedEndAddress, 1 * sizeof(u8),
			  REG_u8);
    /* rythm(ADPCMA) */
